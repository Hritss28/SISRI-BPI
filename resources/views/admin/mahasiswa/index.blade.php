<x-app-layout>
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Kelola data mahasiswa di sistem</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-4 mb-6">
            <form action="{{ route('admin.mahasiswa.index') }}" method="GET" class="flex flex-wrap gap-4">
                ...
                @if(request()->hasAny(['search', 'prodi', 'status']))
                    <a href="{{ route('admin.mahasiswa.index') }}" class="px-4 py-2 text-gray-500 hover:text-gray-700 text-sm">
                        Reset
                    </a>
                @endif
            </form>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($mahasiswa as $index => $mhs)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center">
                                                    <span class="text-blue-600 font-medium text-sm">{{ substr($mhs->user->name ?? $mhs->nama ?? 'M', 0, 1) }}</span>
                                                </div>
                                                <div class="ml-4">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $mhs->prodi->nama ?? '-' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $mhs->angkatan }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if($mhs->user->is_active ?? false)
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center gap-2">
                                                <a href="{{ route('admin.mahasiswa.show', $mhs) }}" class="text-blue-600 hover:text-blue-900" title="Detail">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                                    </svg>
                                                </a>
                                                <a href="{{ route('admin.mahasiswa.edit', $mhs) }}" class="text-yellow-600 hover:text-yellow-900" title="Edit">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                                    </svg>
                                                </a>
                                                <button type="button" onclick="openResetPasswordModal({{ $mhs->user->id }}, '{{ $mhs->user->name ?? $mhs->nama }}')" 
                                                    class="text-purple-600 hover:text-purple-900" title="Reset Password">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"></path>
                                                    </svg>
                                                </button>
                                                <form action="{{ route('admin.mahasiswa.destroy', $mhs) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus mahasiswa ini?')">
                                                    @csrf
                                                    @method('DELETE')
                ...
                <div class="p-6">
                    ...
                    <form id="resetPasswordForm" action="" method="POST">
                        ...
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <label for="useDefaultPassword" class="text-sm text-gray-700">Gunakan NIM sebagai password default</label>
                        </div>
    ...
    <script>
        function openResetPasswordModal(userId, userName) {
            ...
            document.getElementById('resetPasswordForm').action = '/admin/mahasiswa/' + userId + '/reset-password';
        }
