<x-app-layout>
    <div class="max-w-3xl mx-auto">
        ...
        <div class="mb-6">
            @php
                $backUrl = route('admin.unit.index', ['type' => $unit->type]);
                $pageTitle = match($unit->type) {
            ...
            <p class="text-sm text-gray-500 mt-1">Ubah data unit</p>
        </div>

        <!-- Form -->
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <form action="{{ route('admin.unit.update', $unit) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select id="type" name="type" required
                        ...
                        <option value="jurusan" {{ old('type', $unit->type) === 'jurusan' ? 'selected' : '' }}>Jurusan</option>
                        <option value="prodi" {{ old('type', $unit->type) === 'prodi' ? 'selected' : '' }}>Program Studi</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
