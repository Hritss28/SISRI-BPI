    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Buat Jadwal {{ $jenis === 'sempro' ? 'Seminar Proposal' : 'Sidang Skripsi' }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('koordinator.penjadwalan.store') }}" method="POST">
                        @csrf
                        <input type="hidden" name="jenis" value="{{ $jenis === 'sempro' ? 'seminar_proposal' : 'sidang_skripsi' }}">

                        <div class="mb-4">
                            <label for="nama" class="block text-sm font-medium text-gray-700">Nama Jadwal</label>
                            <input type="text" name="nama" id="nama" value="{{ old('nama') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                placeholder="Contoh: {{ $jenis === 'sempro' ? 'Sempro' : 'Sidang' }} Periode I 2024/2025" required>
                            @error('nama')
                        ...
                        <div class="flex items-center justify-end space-x-3">
                            <a href="{{ route('koordinator.penjadwalan.index', ['jenis' => $jenis]) }}"
